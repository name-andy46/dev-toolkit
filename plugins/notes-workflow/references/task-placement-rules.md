...
Shared by every skill in this plugin that writes to the command center —
`update-daily-note` (warm, in the session that did the work) and
`cleanup-daily-note` (cold, evening). Both apply the identical rules, which is
why they live here rather than in either skill: two copies drift, and a task file
governed by two different rulebooks is exactly the failure these rules exist to
prevent.
...
This gate is why any skill writing here must read `current_tasks.md` in **full**
